...
struct ClassLibrary *ButtonBase = NULL, *BitMapBase = NULL, *CheckBoxBase = NULL,
                    ...
                    *GetFileBase = NULL, *SliderBase = NULL;
// the class pointer
...
DBUG("OpenLibs() - START\n",NULL);
	   ...
	   &&  (ILocale=(struct LocaleIFace *)IExec->GetInterface(LocaleBase, "main", 1, NULL)) )
	{
		...
		li.li_Catalog = ILocale->OpenCatalog(NULL, "mgbagui.catalog",
...
//	StringBase = IIntuition->OpenClass("gadgets/string.gadget", 52, &StringClass);
	...
	if(RequesterBase==NULL  ||  IntegerBase==NULL  ||  SpaceBase==NULL  /*||  StringBase==NULL*/
	   ...
	   ||  SliderBase==NULL  ||  LayoutBase==NULL  ||  ChooserBase==NULL) { return FALSE; }
...
int32 GetRoms(STRPTR const romsdir, struct List *list)
{
	APTR context;
	int32 resul = 0;
...
DBUG("GetRoms() '%s'\n",romsdir);
	...
	IDOS->ParsePatternNoCase("#?.(7zip|7z|zip|gb|gbc|gba)", pattern_ms, 64);
...
DBUG("ttpArg = '%s'\n",ttpArg);
					// Disable tooltype: tooltype[=value] -> (tooltype[=value])
					if(ttpArg[0]==DISABLE_TT_CHAR  &&  micon->do_ToolTypes[i][0]!='(')
					{
						//ttpArg  = NULL;
						IUtility->SNPrintf(newttp, MAX_DOS_PATH, "(%s)",micon->do_ToolTypes[i]);
DBUG("  Tooltype disabled: %s' -> '%s'\n",micon->do_ToolTypes[i],newttp);
						//oldttp = micon->do_ToolTypes;
						micon->do_ToolTypes[i] = newttp;
					...
					// Normal tooltype: tooltype=value OR (tooltype=value)
					if(ttpArg[0]!=DISABLE_TT_CHAR  &&  ttpArg  &&  IUtility->Stricmp(micon->do_ToolTypes[i],newttp) )
...
DBUG("  Tooltype modified: '%s'\n",newttp);
					...
					// Toggle/Switch tooltype: 'tooltype' OR '(tooltype)'
...
void GetSavestates(struct MGBAGUI *gui, STRPTR fn)
{
	APTR context;
	struct Node *node;
...
DBUG("GetSavestates()\n",NULL);
	...
	IUtility->Strlcat(filestate, ".ss[2-9]", MAX_DOS_FILENAME);
DBUG("  searching \"%s\":\n",filestate);
	// Detach chooser list
...
DBUG("    slot #%s (0x%lx)\n",slot,slot[0]);
				...
				node = IChooser->AllocChooserNode(CNA_CopyText,TRUE, CNA_Text,slot, CNA_UserData,slot[0], TAG_DONE);
				if(node) { IExec->AddTail(gui->savestates_list, node); }
			}
		}
//		if(IDOS->IoErr() == ERROR_NO_MORE_ENTRIES) {
...
DBUG("  SortList() savestates\n",NULL);
		IUtility->SortList(gui->savestates_list, &HK);
//			}
//		}
	}
...
void ShowPreview(struct MGBAGUI *gui)
{
	uint32 res_n;
...
void LaunchRom(struct MGBAGUI *gui)
{
	uint32 res_n;
...
DBUG("  res_n=0x%08lx -> res_s='%s' (res_e='%s')\n",res_n,res_s,res_e);
	...
	IUtility->Strlcpy(filename, gui->myTT.romsdrawer, FILENAME_LENGTH);
...
//DBUG("  gui->wbs = 0x%08lx\n",gui->wbs);
		if(gui->wbs)
		{
			IIntuition->GetAttrs(OBJ(OID_LISTBROWSER), LISTBROWSER_Selected,&gui->myTT.last_rom_run, TAG_DONE);
			++gui->myTT.last_rom_run; // listbrowser starts from 0
DBUG("last_rom_run = %ld\n",gui->myTT.last_rom_run);
			IUtility->SNPrintf(rom_ext, sizeof(rom_ext), "%ld",gui->myTT.last_rom_run); // using 'rom_ext' as temp buffer
			SaveToolType(gui->wbs->sm_ArgList->wa_Name, "LAST_ROM_LAUNCHED", rom_ext);
		}
	}
